...
const { search } = require('./middleware/touchSession');
 ...
 * @param uploadFolder
 * @param projectJSON
 * @returns {{loadProject: loadProject, createNewProject: createNewProject, createNewDirectory: createNewDirectory, saveProjectDescription: saveProjectDescription, renameCategory: renameCategory, removeCategory: removeCategory, deleteProject: deleteProject, deleteFolder: deleteFolder, saveKey: saveKey, cloneKey: cloneKey, saveBundle: saveBundle, importJSON: importJSON, renameKey: renameKey, removeKey: removeKey, getDirectory: getDirectory, addImage: addImage, removeImage: removeImage, setupClient: setupClient, searchText: searchText}}
 */
var dao = function ({
  projectFolder,
  uploadFolder,
  ...
  const api = {
    ...
    /**
     * Retrieve a list of all projects listed inside the projects.json
     * (indexing file containing meta data related to all translation projects on the server)
     *
     * @param   {[type]}  callback provided by operations <- trade
     */
    getTableOfContents: function (callback) {
      projectHandler
        .read()
        .then(function (data) {
          callback && callback(null, data.projects);
        })
        .catch(function (err) {
     ...
     * @param cb
     */
    importJSON: function (projectId, data, cb, preventOverride) {
      this.loadProject(projectId, (projectData, { id, name, url, file }) => {
        if (!projectData) {
        ...
        var localKeys = projectData.keys,
          ...
          hasValidLocales =
            availableLanguages.filter(
              (lang) => uploadedLanguages.indexOf(lang) !== -1
            ).length > 0,
          currentLanguage,
          localLangKeys,
          uploadedLangKeys,
        ...
        if (uploadedLanguages.length === 0 || !hasValidLocales) {
          cb(
            new Error(
              'Invalid format: No valid locales defined at first level.'
            )
          );
          return;
        }
          ...
          if (availableLanguages.indexOf(currentLanguage) > -1) {
            ...
            for (var key in uploadedLangKeys) {
              if (uploadedLangKeys.hasOwnProperty(key)) {
                ...
                var duplicates = Object.keys(currentKey).filter((key) =>
                  localLangKeys.hasOwnProperty(key)
                );

                if (duplicates.length > 0 && preventOverride === true) {
        ...
        if (keyDupes.length && preventOverride === true) {
          cb(new Error('Duplicated keys not allowed'), projectId, [
            ...new Set(keyDupes),
          ]);
          return;
        }
    ...
    /**
     * Copy the translation keys of a source project into a target project
     * defined by the projects ids
     *
     * @param targetProject
     * @param project2Copy
     * @param callback
     */
    copyProject: function (targetProject, project2Copy, callback) {
      this.loadProject(project2Copy.id, (importContent) => {
        this.importJSON(targetProject.id, importContent.keys, callback, true);
      });
    },
    /**
     * TODO test projectHandler
  ...
  if (searchConfig && searchConfig.socket) {
    const UnixClient = require('./unixClient');
    const unixClient = new UnixClient(projectFolder, searchConfig.socket);
    api.searchTerm = function (searchTerm, cb) {
      unixClient.search(searchTerm, cb);
    };
    api.invalidateSearch = function () {
      unixClient.invalidate();
    };
  }
