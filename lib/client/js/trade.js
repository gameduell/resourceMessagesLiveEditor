...
var trade = (function () {
    ...
    function init(s, fc) {
        ...
        if (server.setUserRights) {
            server.setUserRights(
                canny.cookieManager
                    .forSessionCookie('translatron_session')
                    .getValues(),
                fc
            )
        } else {
    ...
    function createConnection(fc) {
        ...
        d.on('end', function (err) {
            ...
            if (tryToReconnect) connectionLostUi.render('RECONNECT_FAIL')
                ...
                setTimeout(
                    () => connectionLostUi.render('CONNECTION_END'),
                    1000
                )
        })
    ...
    return {
            ...
            if (isConnected())
                server.createNewProject(
                    ...
                    function (err, projectData, project) {
                        // TODO handle error case
                        if (!err) {
                            callController('onNewProjectCreated', [
                                projectData,
                                project,
                            ])
                        }
        ...
        createNewDirectory: function (directoryName, currentDirectory) {
            if (isConnected())
                server.createNewDirectory(
                    directoryName,
                    currentDirectory,
                    function (err, directoryData) {
                        // TODO handle error case
                        if (!err) {
                            callController('onNewDirectoryCreated', [
                                directoryData,
                            ])
                        }
                    }
        ...
        searchTerm: function (searchTerm, callback) {
            if (isConnected())
                server.searchTerm(searchTerm, function (err, results) {
                    if (!err) {
        ...
        importJSON: function (targetProject, transKeys, options, callback) {
            if (isConnected())
                server.importJSON(
                    targetProject.id,
                    transKeys,
                    function (err, projectId, projectData) {
                        if (!err) {
                            callController('jsonImported', [
                                projectId,
                                projectData,
                            ])
                        } else {
                            callback && callback(err, projectId, projectData)
                        }
                    },
                    options.preventOverride
                )
        },
        getTableOfContents: function (callback) {
            if (isConnected())
                server.getTableOfContents(function (err, data) {
                    if (!err) {
                        callController('getTableOfContents', [data])
                    } else {
                        callback && callback(err)
                    }
                })
